...
const calcularMonto = async (req, res) => {
  try {
    ...
    res.json({
      ...
      descuento_pct,
      recargo_pct,
      monto_final: Math.round(monto_final),
      ahorro: Math.round(monto_original - monto_final)
    })
  } catch (error) {
...
const crearPago = async (req, res) => {
  try {
    ...
    const pedido = await db.query('SELECT total, estado FROM pedidos WHERE id = $1', [pedido_id])
    if (pedido.rows.length === 0) {
      return res.status(404).json({ error: 'Pedido no encontrado' })
    }
    ...
    const result = await db.query(
      `INSERT INTO pagos (pedido_id, metodo, monto_original, descuento_pct, recargo_pct, monto_final, estado)
    ...
    if (metodo === 'transferencia') {
      msgPago = `Datos para transferir:\nCBU: ${cbu}\nAlias: ${alias}\nTitular: ${titular}`
    } else if (metodo === 'mercadopago') {
      msgPago = `Podés pagar por MercadoPago. Te enviamos el link a la brevedad.`
    } else if (metodo === 'efectivo') {
      msgPago = `Abonás en efectivo al retirar.`
    }

    const pedidoData = await db.query(
      `SELECT p.codigo, u.nombre FROM pedidos p LEFT JOIN usuarios u ON p.usuario_id = u.id WHERE p.id = $1`,
      [pedido_id]
    )
    const { codigo, nombre } = pedidoData.rows[0]

    const mensajeWA = `Hola ${nombre}! 📷 Tu pedido *${codigo}* fue recibido.\n\n` +
      `*Total a abonar: $${monto_final.toLocaleString()}*` +
      (descuento_pct > 0 ? ` (${descuento_pct}% de descuento aplicado)` : '') +
      (recargo_pct > 0 ? ` (incluye ${recargo_pct}% de recargo)` : '') +
      `\n\n${msgPago}\n\n` +
      `⚠️ El procesamiento comienza una vez confirmado el pago.`
...
const confirmarComprobante = async (req, res) => {
  try {
    ...
    const pago = await db.query(
      'UPDATE pagos SET estado = $1, comprobante_url = $2 WHERE id = $3 RETURNING *',
      ['confirmado', comprobante_url, id]
    )
    ...
    await db.query(
      ...
      ['en_proceso', pago.rows[0].pedido_id]
    )

    const pedidoData = await db.query(
      `SELECT p.codigo, u.nombre FROM pedidos p LEFT JOIN usuarios u ON p.usuario_id = u.id WHERE p.id = $1`,
      [pago.rows[0].pedido_id]
    )
    const { codigo, nombre } = pedidoData.rows[0]

    const mensajeWA = `Hola ${nombre}! ✅ Recibimos tu comprobante de pago.\n\n` +
      `Tu pedido *${codigo}* está siendo procesado. 📸\n\n` +
...
const crearPromo = async (req, res) => {
  try {
    ...
    const result = await db.query(
      'INSERT INTO promos (nombre, descripcion, precio) VALUES ($1, $2, $3) RETURNING *',
